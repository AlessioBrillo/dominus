...
export function reportProviderStatuses(
  ...
  return [
    ...
    },
    {
      name: 'RDAP',
      configured: true,
      note: `Public RDAP confirmation via rdap.org universal + IANA per-TLD bootstrap (exponential backoff, ADR-0058)${config.RDAP_BOOTSTRAP_URLS ? ' (custom RDAP_BOOTSTRAP_URLS configured)' : ''}.${config.RDAP_CONSENSUS_ENABLED ? ` 2-of-2 consensus gate on ${config.RDAP_CONSENSUS_ENDPOINT} with per-TLD origin-overlap guard (ADR-0058)${config.RDAP_CONSENSUS_RESCUE_WHOIS_ENABLED ? ' and WHOIS rescue leg (ADR-0051)' : ''}.` : ''}`,
    },
    {
      name: 'USPTO',
      configured: true,
