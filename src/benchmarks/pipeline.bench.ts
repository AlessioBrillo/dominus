  ...
  return {
    ...
    checkBulk: vi
      .fn<
        (
          domains: string[],
        ) => Promise<Array<{ domain: string; status: DomainStatus; checkedAt: string }>>
      >()
...
describe('PipelineOrchestrator throughput', () => {
  ...
  bench('5 candidates', async () => {
    await orchestrator.run({ brandableNames: ['a.com', 'b.com', 'c.com', 'd.com', 'e.com'] });
  });

  bench('50 candidates', async () => {
    const names = Array.from({ length: 50 }, (_, i) => `${i}.com`);
