...
import type { CreateCliOptions } from '../index.js';
...
function mockConfig(): Config {
  return {
    ...
    DNS_CONSENSUS_RATE_LIMIT_TOKENS_1: 10,
    DNS_CONSENSUS_RATE_LIMIT_INTERVAL_MS_1: 1000,
    DNS_CONSENSUS_RATE_LIMIT_TOKENS_2: 10,
    DNS_CONSENSUS_RATE_LIMIT_INTERVAL_MS_2: 1000,
...
function makeCliOptions(
  db: Database.Database,
  overrides: Partial<CreateCliOptions> = {},
): CreateCliOptions {
  return {
    db,
    runService: {} as unknown as PipelineRunService,
    manager: { setRescoreService: vi.fn() } as unknown as PortfolioManager,
    engine: {} as unknown as ScoringEngine,
    outcomeRepo: {} as never,
    config: mockConfig(),
    candidateRepo: {} as never,
    scoringRepo: {} as never,
    trademarkRepo: {} as never,
    runsRepo: {} as never,
    scheduler: undefined,
    ...overrides,
  };
...
describe('CLI command registration', () => {
  ...
  it('creates a Command with name dominus', () => {
    const cli = createCli(makeCliOptions(testDb));
    expect(cli.name()).toBe('dominus');
  });
  ...
  it('registers the portfolio command', () => {
    const cli = createCli(makeCliOptions(testDb));
    const cmd = cli.commands.find((c) => c.name() === 'portfolio');
    expect(cmd).toBeDefined();
  });

  it('registers the candidates command', () => {
    const cli = createCli(makeCliOptions(testDb));
    const cmd = cli.commands.find((c) => c.name() === 'candidates');
    expect(cmd).toBeDefined();
  });

  it('registers core commands (score, outcome, backtest, runs, maintenance, providers, health)', () => {
    const cli = createCli(makeCliOptions(testDb));
  ...
  it('registers scheduler command when scheduler is provided', () => {
    const cli = createCli(
      makeCliOptions(testDb, {
        scheduler: {} as never,
      }),
    );
    const names = cli.commands.map((c) => c.name());
  ...
  it('does not register scheduler command when scheduler is undefined', () => {
    const cli = createCli(
      makeCliOptions(testDb, {
        scheduler: undefined,
      }),
  ...
  it('registers watchlist command when watchlistService is provided', () => {
    const cli = createCli(
      makeCliOptions(testDb, {
  ...
  it('registers buy + registrars commands when purchaseService is provided', () => {
    const cli = createCli(
      makeCliOptions(testDb, {
        purchaseService: { registrarName: 'manual' } as never,
      }),
    );
    const names = cli.commands.map((c) => c.name());
    expect(names).toContain('buy');
    expect(names).toContain('registrars');
  });

  it('registers report command when reportService is provided', () => {
    const cli = createCli(
      makeCliOptions(testDb, {
  ...
  it('registers analytics command when accuracyAnalyzer is provided', () => {
    const cli = createCli(
      makeCliOptions(testDb, {
        accuracyAnalyzer: {} as never,
      }),
    );
    const names = cli.commands.map((c) => c.name());
  ...
  it('registers bid command when acquisitionService is provided', () => {
    const cli = createCli(
      makeCliOptions(testDb, {
  ...
  it('registers listing command when listingManager is provided', () => {
    const cli = createCli(
      makeCliOptions(testDb, {
        listingManager: {} as never,
      }),
    );
    const names = cli.commands.map((c) => c.name());
