...
export function registerRunCommand(program: Command, deps: RunCommandDeps): void {
  ...
  const run = program
    ...
    .option(
      '--sync',
      'Run synchronously and wait for completion (default: async via job queue when available)',
      false,
    )
    .option(
      '--wait',
  ...
  run.action(
    async (options: {
      ...
      if (useSync || !canEnqueue) {
        if (!canEnqueue && !useSync) {
          process.stdout.write(
            'Job queue not available — falling back to synchronous execution.\n',
          );
        }
      ...
      if (useWait) {
        ...
        if (deps.runsRepo) {
          ...
          if (!Number.isNaN(parsedJobId)) {
            const poll = setInterval(async () => {
              const status = await jobQueueService!.getJobStatus(parsedJobId);
              if (!status) return;
