...
describe('TrademarkRepository', () => {
  ...
  describe('pruneExpired', () => {
    it('deletes only rows whose expires_at is in the past', async () => {
      // Arrange
      await repo.insertByTerm('fresh', 'USPTO', false, [], { hits: [] }, 7);
      await repo.insertByTerm('expired-1', 'USPTO', false, [], { hits: [] }, -1);
      await repo.insertByTerm('expired-2', 'EUIPO', false, [], { hits: [] }, -2);
    ...
    it('is idempotent — a second call is a no-op', async () => {
      // Arrange
      await repo.insertByTerm('expired', 'USPTO', false, [], { hits: [] }, -1);

      // Act
      const first = await repo.pruneExpired();
      const second = await repo.pruneExpired();
    ...
    it('respects the optional "now" cutoff for deterministic tests', async () => {
      // Arrange
      await repo.insertByTerm('old', 'USPTO', false, [], { hits: [] }, -1);

      // Act — pretend "now" is before the row's expires_at, so the row is kept
      const removed = await repo.pruneExpired('2020-01-01T00:00:00.000Z');
