...
export class PipelineOrchestrator {
  constructor(
    ...
    private readonly trademarkStage: TrademarkGateStage,
  ) {}
  ...
  async run(input: CandidateGenerationInput): Promise<PipelineResult> {
    ...
    const trademark = await this.trademarkStage.process(rdap.passed);
    ...
    const scoring = await this.scoringStage.process(trademark.passed);
