...
describe('ExpirySignal', () => {
  it('returns 0 for non-closeout domains', () => {
    const result = computeExpiryScore({ domain: 'nova.com', tld: '.com', sld: 'nova', isCloseout: false }, 1);
    expect(result.score).toBe(0);
  });

  it('closeout with age + backlinks scores > 0', () => {
    const result = computeExpiryScore(
      { domain: 'old.com', tld: '.com', sld: 'old', isCloseout: true, domainAge: 10, backlinks: 200 },
      1,
    );
  ...
  it('closeout without data scores 0', () => {
    const result = computeExpiryScore({ domain: 'old.com', tld: '.com', sld: 'old', isCloseout: true }, 1);
    expect(result.score).toBe(0);
  });
  ...
  it('waybackSnapshots contributes to score', () => {
    const result = computeExpiryScore(
      { domain: 'archived.com', tld: '.com', sld: 'archived', isCloseout: true, waybackSnapshots: 100 },
      1,
    );
    expect(result.score).toBeGreaterThan(0);
  });
  ...
  it('score is clamped between 0 and 1', () => {
    const result = computeExpiryScore(
      { domain: 'old.com', tld: '.com', sld: 'old', isCloseout: true, domainAge: 100, backlinks: 100000 },
      1,
    );
    expect(result.score).toBeLessThanOrEqual(1);
  ...
  it('extreme values cap at 1.0', () => {
    const result = computeExpiryScore(
      { domain: 'extreme.com', tld: '.com', sld: 'extreme', isCloseout: true, domainAge: 50, backlinks: 5000, waybackSnapshots: 2000 },
      1,
    );
